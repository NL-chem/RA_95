...
all_ligands=(NIR NIS NKR NKS PIR PIS PPK PPS)
	...
	for ligand in ${all_ligands[@]}; do
		...
		then
			...
			for i in $(seq 1 $iteration); do
				bsub -n 1 -W $dur /cluster/apps/rosetta/3.11/x86_64/main/source/bin/rosetta_scripts.hdf5.linuxgccrelease @$flags -out:suffix _$i -out:path:all run/$ligand -in:file:s pdb_inp/$pdb_input -extra_res_fa ligs/$params -enzdes:cstfile ligs/$cst
				counter+=1
			done
